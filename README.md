...
- 用户、用量、上游配置状态、目录定价和支付复核管理后台
- D1 用户、API Key、不可变余额账本、支付订单、端点目录和调用日志
- API Key 仅创建时展示明文；数据库只保存 SHA-256 哈希
- 同步 TikHub OpenAPI 与价格目录中的全部服务类型；新端点默认禁用
- 后台可按数据类型/路径设置客户价与上下架；高风险或无可验证价格的端点强制关闭
...
`drizzle/0000_*.sql` 至 `drizzle/0008_*.sql`，必须按编号顺序执行。`0007`
会无损重建已有的支付事件表，以支持失败事件退避重试。
...
同步把 OpenAPI 中的全部路径带入后台，包括 GET/POST 方法、摘要、描述与参数
schema；未在可信价格目录中找到成本的路径会标记为“价格未验证”，不能启用。
...
6. 定时对账会重试未处理事件并轮询待确认订单，弥补回调丢失。
7. 少付、错币、错链、晚到款、重复入金和超过支付商金额 1 bp 容差的多付进入
   唯一的资金复核案件；普通订单恢复会
   被阻止，避免绕过案件。
8. `GET /api/admin/payment-reviews` 查看证据；只有再次向支付商查询后，才可用
   `POST /api/admin/payment-reviews/resolve` 明确入账、确认退款或拒绝。
9. 人工入账以案件级唯一账本引用保证幂等；后续服务商退款只冲销该笔付款对应的
   正常/人工入账，不会误冲其他重复入金。
10. 对账除待确认订单外，还会轮询最近 180 天内仍有净入账的父付款与重复入金
    子付款；即使退款 IPN 丢失，也会按付款级轮转游标发现并冲销。

...
> 当前仓库默认是安全沙盒。未取得 TikHub 经销/白标书面授权、TikHub 对稳定币
> 仅作为 API 服务付款方式的书面澄清、商户审核和适用地区的法律意见前，不要
> 打开真实代理或真实加密收款。

当前应用版本：`v0.3.0-preview.6` · API 契约版本：`v1`
...
- 用户、用量、登录身份、活跃会话/API Key、TikHub 加密数据源、目录定价和支付
  复核管理后台
- D1 用户、API Key、数据库强制只追加的余额账本、保留支付订单、端点目录和调用日志
- API Key 仅创建时展示明文；数据库只保存 SHA-256 哈希
- TikHub V5.3.2 静态参考市场收录 1,025 个 GET/POST 数据操作，并提供搜索、
  多维筛选、服务端分页和同页参数/示例详情
- 同步 TikHub OpenAPI 路径、官方 tags、`operationId`、归一化数据类型、调用表面与
  可信价格目录；新端点默认禁用
- 后台可按路径设置客户价与上下架，也可先冻结服务端筛选结果，再原子批量调价、
  上架或下架；批量选择器支持平台、数据类型、官方 tag、调用表面、状态与安全分类，
  高风险或无可验证价格的端点强制关闭
...
- 丢失支付回调的主动轮询对账、Worker 定时处理入口，以及刷新后可恢复的充值地址
  和 invoice
- 重复入金、少付和异常终态的持久化人工复核、审计与精确退款冲销
- TikHub 转售授权、稳定币付款方式书面澄清和加密支付法律审查三重合规门禁
...
`drizzle/0000_*.sql` 至 `drizzle/0013_*.sql`，必须按编号顺序执行。`0007`
会无损重建已有的支付事件表，以支持失败事件退避重试；`0009`–`0010`
增加加密上游凭据、唯一活动指针和目录凭据代次绑定；`0011` 保存目录覆盖计数和
上下游快照哈希；`0012` 增加机器安全分类、端点 revision，以及不可变批量预览与
应用回执；`0013` 将 `dataType`、官方 tags、调用表面和 `operationId` 持久化到
同步暂存、实时目录与批量计划。`0012` 和 `0013` 都会安全下架旧目录；`0013`
还会清除未完成的同步暂存、租约和旧成功代次，并创建数据库触发器阻止余额账本
UPDATE/DELETE 与支付订单 DELETE，避免用户级联删除破坏财务证据。迁移后必须重新
全量同步并重新审核，不能把迁移时的 `other` / `[]` 占位值直接视为已核验分类。
...
- 正式生产 readiness 要求 Google 与钱包登录同时配置；受信任 Sites 身份只作为
  私有预览回退，不能替代这两个客户登录入口。
- 首次登录的用户与身份关联在同一 D1 事务中创建；身份写入失败不会留下阻止重试的
  孤立用户。新 Google subject 也不会仅凭同名邮箱接管已有账户；冲突必须由支持
  团队先完成独立身份核验，应用不会自动关联。
- 管理员把用户设为 `suspended` 时，用户状态、全部客户 API Key 撤销、全部登录
  session 删除和管理员审计在同一 D1 原子批次提交；状态更新使用旧状态 CAS。登录
  或建 Key 的并发写入也会在同一事务内重新确认用户仍为 active，因此重新启用账户
  不会恢复旧凭据。
...
- `TIKHUB_CRYPTO_PAYMENT_CLEARED=true`：只在 TikHub 书面确认其业务限制不禁止
  把稳定币仅作为 API 服务付款方式、且证据已归档后设置。
- `LEGAL_REVIEW_CONFIRMED=true`：运营主体和目标客户地区完成法律审查后设置。

四个条件以及数据库、Google 与钱包两种登录方式、TikHub/支付商密钥、管理密钥、
已审核目录和五分钟内的对账心跳未全部满足时，对应真实操作会安全失败。支付创建
要求至少有一个已审核开放的接口，避免“可以充值但没有服务可用”。
...
开放目录支持精确的 `platform`、`dataType`、`tag` 与 `surface` 筛选，并返回与
当前成功同步代次绑定的 `dataType`、`tags`、`surface` 和 `operationId`。分类或
官方 tag 与静态参考快照不一致时，参考市场不会叠加实时价格或 `available` 状态。
同步遇到畸形 v1 operation、归一化路径冲突或疑似密钥/Token 的分类标签时会整次
失败并保留上一成功目录；`/api/health` 与 `/api/readiness` 的
`capabilities.taxonomyReady` 只有在实时分类通过严格 canonical 校验时才为 true。
...
同步把 OpenAPI 中识别到的路径带入后台，包括 GET/POST 方法、摘要、描述、参数
schema、官方 tags、`operationId`、RelayBase 归一化 `dataType` 与
`app|web|app_web|other` 调用表面。所有实时分类使用与静态参考生成器相同的确定性
规则；非法、超限或不完整的 v1 operation 元数据会让整轮同步失败，不会写入默认
分类后继续发布。只有同时存在于 OpenAPI、方法匹配、位于活动 Key 的已验证 scope
内，且在可信价格目录中找到成本的路径，才会标记为“价格已验证”；其他路径不能启用。
...
用前端估算冒充全量同步。部署 `0011`、`0012` 或 `0013` 后必须成功重新同步一次；
旧目录没有完整覆盖与分类证据，或证据计数/哈希不自洽时，readiness 与真实代理都会
fail closed。
...
管理后台“路由与定价”会把当前平台、精确数据类型、精确官方 tag、调用表面、搜索、
状态与安全分类作为服务端选择器。“生成预览”后，服务端冻结每个端点的 revision、
分类元数据、成本、客户价、审核状态、目录代次、活动数据源指纹和 OpenAPI/价格快照
哈希，并返回需手输的确认文本。预览默认 30 分钟过期；批量上架最多 500 个端点，
调价或下架最多 2,000 个，超过时必须缩小筛选范围。
...
6. 定时对账会重试未处理事件并轮询待确认订单，弥补回调丢失；最近 7 天内的
   `expired` 或零入账 `failed` 原始订单也会每 6 小时复查，捕获支付商状态变化和
   晚到款。对“已拒绝且该付款从未入账”的案件，会在结案 6 小时后开始低频复查。
   同一付款成功查询后至少间隔 6 小时，服务商错误至少间隔 1 小时；候选和并发均
   有硬上限，避免无限扫描或过度调用。
...
10. 人工入账以案件级唯一账本引用保证幂等；后续服务商退款只冲销该笔付款对应的
   正常/人工入账，不会误冲其他重复入金。
11. 对账除待确认订单外，还会持续轮询所有仍有净入账的父付款与重复入金子付款；
    候选以付款级正向账本和未冲销状态为准，不依赖父子付款共享的订单状态。即使
    入账超过 180 天、子付款仍在人工复核且父付款退款 IPN 丢失，也会发现并精确
    冲销。
12. 如果本轮确实调用了支付 provider、但所有 provider 查询都失败，对账返回
    `502`，只记录失败诊断，不刷新成功的 `reconciliation` 心跳；readiness 会随
    旧成功心跳过期而自动关闭真实代理和充值。
13. 用户控制台的最近充值会分别显示订单面值、实际入账、退款冲销和最近复核原因；
    管理后台可输入已核验的 RelayBase 订单号与 NOWPayments `payment_id`，恢复
    “服务商已创建但本地未绑定”的订单，恢复过程仍会重新查询服务商并执行全部
    金额、币种与案件门禁。

Worker 已实现 `scheduled` 处理入口；生产托管仍必须显式启用每分钟 Cron Trigger，
或由受保护的外部调度器调用 `POST /api/admin/reconcile`。此外仍需增加 AML/制裁
筛查、按主体的充值限额、人工退款审批和异常支付处置流程。
...
- TikHub 书面确认其“虚拟货币及相关行业”业务限制不禁止把稳定币仅作为 API 服务
  付款方式；确认前真实加密收款保持关闭
...
- 每分钟启用 Worker `scheduled` Cron Trigger；若托管平台不支持，则由受保护的
  外部调度器调用 `POST /api/admin/reconcile`。两种方式都使用独立
  `RECONCILIATION_SECRET`，回退两分钟前仍未完成但已扣费的代理请求，并处理支付
  事件和轮询待确认充值
...
- `GET /api/readiness` 只有数据库迁移、Google 与钱包登录、转售/法律/加密付款
  书面澄清门控、上游、支付、管理、已审核目录和近期对账心跳全部就绪时返回
  `200`；未就绪返回 `503`。
